...
class StudentStorage {
  ...
  try {
    final response = await SupabaseConfig.supabase.auth.signUp(
      email: student.email,
      password: 'example-password', // Substitua por student.password se necessário
    ...
    if (user != null) {
      ...
      if (saveResult != null) {
        return saveResult; // Retorna mensagem de erro, se houver
      }

      return null; // Retorna null para indicar sucesso
    } else {
      return 'Erro ao registrar usuário: usuário não criado.';
    }
  ...
  try {
    ...
    //   print('Erro ao salvar usuário na tabela: ${response.error!.message}');
    //   return 'Erro ao salvar usuário na tabela: ${response.error!.message}';
    // }
